...
print("THe range is between -0.5,-0.4")
...
hcv<-function(simdata,n,K,s,hn=NULL){
  # browser()
  test_idk <- lapply(split(sample(1:n,n),rep(1:K,n/K)),sort)
  ...
  #hdiff <- simdata %>% mutate(diff=X-obs_times) %>% filter(diff>0) %>% group_by(id) %>% mutate(diff1=min(diff)) %>% pull(diff1)
  if(is.null(hn)) {
    hmin <- simdata %>% filter(delta) %>% mutate(a=X-obs_times) %>% filter(a>0) %>% filter(a==min(a)) %>% pull(a)
    hmin <- max(hmin,n^(-0.5))
    hmax <- max(simdata %>% mutate(diff=X-obs_times) %>% filter(diff>0) %>% group_by(id) %>% mutate(diff1=max(diff)) %>% pull(diff1))
    hmax <- min(hmax,n^(-0.4)) 
  ...
  foreach(s_ind = c(5,4,3,2,1)) %do% {
    foreach(cenrate_ind= 1:2) %do% {
      ...
      estres_all_h <- foreach(ii = 1:1000,.packages = c("tidyverse","extraDistr","MASS","nleqslv","nloptr","splines","gaussquad","foreach")) %dopar% {
        ...
        simdata <-
          replicate(
            ...
            simAsytransdata(
              ...
              mu_bar = 8,
              #rep(10,length(tt)),
              alpha = function(tt) s_val + (tt * (1 - sin(2 * pi * (tt - 0.25)))),
        ...
        # unlist(estres_ori)
        # print(simdata)
        hres <- hcv(simdata,nn,5,s_val)
        # browser()
        # print(t(res))
        
        
        hsec <- ifelse(dim(na.omit(hres))[1]==0,nn^(-0.6),na.omit(hres)  %>% filter(cvloss==min(cvloss)) %>% pull(bd)) 
        ...
        #browser()
        list(hres,estres_ori,n=nn,s=s_val,cenrate=cenrate,bd=hsec)
        #browser()
        # list(hres,estres_ori)
        #list(estres_ori)
      }
     
      filename <- paste("/home/dsun30/asyntrans/simres/new6multi","s",s_val,"n",nn,"cen",cenrate,"CV.Rdata",sep="_")
      save(estres_all_h,file=filename)
      
      res_temp <- estres_all_h  %>%
        lapply(function(xx) tibble(est=xx[[2]]$est[1:2],se=xx[[2]]$se[1:2],name=c("gamma1","gamma2"),true_val=c(1,-0.5),bd=xx[[6]])) %>% bind_rows(.id="rep") %>% 
        mutate(lb=est-qnorm(0.975)*se,
               ub=est+qnorm(0.975)*se,
               CP = (lb < true_val)*(ub>true_val)) %>% group_by(name) %>% 
        summarise(bias=mean(est-true_val),sd=sd(est),se=mean(se),CP=mean(CP),bd=mean(bd)) %>% mutate(n=nn,s=s_val,cenrate=cenrate) 
      # res_temp <- estres_all_h  %>%
